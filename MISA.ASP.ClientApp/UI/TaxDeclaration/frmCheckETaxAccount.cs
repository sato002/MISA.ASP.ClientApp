...
{
    public partial class frmCheckETaxAccount : Form
    {
        public frmCheckETaxAccount()
        {
            InitializeComponent();
        ...
        public void ShowForm(string actionID, string data)
        {
            ...
            else
            {
                ...
                {
                    _ = CheckETaxAccount(actionID, data);
                });
            }
        }

        public async Task CheckETaxAccount(string actionID, string data)
        {
            try
            {
                var output = new CheckETaxAccountOutput() { ActionID = actionID };
                var syncInput = JsonConvert.DeserializeObject<CheckETaxAccountInput>(data);
                UpdateProgress(10);
                if (syncInput != null && syncInput.Details != null && syncInput.Details.Count > 0)
                {
                    ...
                    {
                        var iOutputDetail = new CheckETaxAccountOutputDetail() { CustomerID = detail.CustomerID };
                        try
                        {
                            if (!isLocalMode)
                            {
                                ...
                                await crawler.CheckETaxAccount();
                                iOutputDetail.IsValid = true;
                            }
                            else
                            {
                                iOutputDetail.IsValid = true;
                        ...
                        catch (UnRecognizedMainPageException ex)
                        {
                            iOutputDetail.CrawlErrorType = CrawlErrorType.UnRecognizedMainPage;
                            LogUtil.LogError(ex);
                        }
                        catch (Exception ex)
                        {
                ...
                if (output.Details.Count > 0)
                {
                    ...
                    await aspClient.UpdateETaxAccountState(syncInput.ProfileID, output);
                }
        ...
        private void frmCheckETaxAccount_FormClosing(object sender, FormClosingEventArgs e)
        {
            Program.InProcessing = false;
        }
        ...
        private void ShowSuccessForm()
        {
            ...
            {
                var frmSuccess = new frmSuccess("Đã hoàn thành kiểm tra tài khoản và mật khẩu");
        ...
        private void ShowErrorForm()
        {
            ...
            {
                var frmError = new frmError("Đã có lỗi xảy ra, vui lòng thực hiện lại");
                frmError.ShowDialog();
            }
        }
        #endregion
